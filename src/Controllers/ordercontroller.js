...
const createOrder = async function (req, res) {
  try {
    ...
    if (!searchUser) {
      return res
        .status(400)
        .send({ status: false, message: "user doesn't exists for uderId" });
    }

    if (!cartId) {
      return res
        .status(400)
        .send({ status: false, message: "CartId is required field}" });
    }
    ...
    if (cancellable) {
      if (typeof cancellable != "boolean") {
        return res
          ...
          .send({
            status: false,
            message: "cancel must be boolean value true or false",
          });
      }
    }
    ...
    if (!searchCartDetails.items.length) {
      return res
        ...
        .send({
          status: false,
          message:
            "Order already placed for this cart. Please add some products in cart to make an order",
        });
    }
...
const updateOrder = async function (req, res) {
  try {
    ...
    if (!orderId)
      return res
        .status(400)
        .send({ status: false, message: "orderId is required field" });
    if (!isValidObjectId(orderId)) {
      return res
        .status(400)
        .send({ status: false, message: "Invalid orderId in body." });
    }
    ...
    if (!findOrder)
      return res
        ...
        .send({
          status: false,
          message: "order not found with this UserId and OrderId",
        });
    ...
    if (findOrder.cancellable) {
      if (isDeleted == true) {
        let updatedOrder = await orderModel.findOneAndUpdate(
          { _id: orderId, userId },
          { isDeleted, status, deletedAt: Date.now() },
          { new: true }
        );
        return res
          .status(200)
          .send({
            status: true,
            message: "sucessfully Order updated ",
            data: updatedOrder,
          });
      }
    ...
    if (!findOrder.cancellable && status == "cancelled")
      return res
        ...
        .send({
          status: false,
          message: "cant modify status to cancelled,as cancellable is false",
        });
